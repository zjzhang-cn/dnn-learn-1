# DNN 深度学习入门示例

通过 PyTorch 实现的深度神经网络（DNN）教学项目，用小型实验展示神经网络的核心概念。

## 项目简介

本项目包含一个 **DNN 符号分类器**，通过三层全连接网络判断 16 位有符号整数是正数还是负数。虽然问题本身很简单（只需看最高位），但作为 DNN 入门示例，它完整展示了：

- 数据预处理（整数 → 16 位二进制特征）
- 模型定义（`nn.Module` 子类化）
- 训练循环（前向传播 → 损失计算 → 反向传播 → 参数更新）
- 验证评估（准确率、损失曲线）
- 权重分析（验证网络学到了什么）
- 交互式推理

## 环境要求

- Python 3.12+
- PyTorch >= 2.0
- NumPy >= 1.24

## 快速开始

```bash
# 1. 安装依赖
pip install -r requirements.txt

# 2. 训练模型
python train.py

# 3. 运行推理（交互式）
python inference.py
```

程序会自动：
1. 生成 20000 条训练数据
2. 构建并训练神经网络，保存模型到 `sign_classifier.pth`
3. 打印权重分析（展示网络学会了 MSB 是关键特征）
4. 进入交互模式，等待用户输入整数进行预测

## 交互示例

```
请输入一个整数: 100
  预测: 正数  |  实际: 正数（>=0）  |  置信度: 99.84%  ✅

请输入一个整数: -255
  预测: 负数  |  实际: 负数（<0）  |  置信度: 100.00%  ✅

请输入一个整数: q
再见！
```

## 模型结构

```
输入层 (16)               ← 16 位二进制表示（补码，MSB 在前）
    ↓
全连接层 (32) + ReLU      ← 扩展到 32 维，学习特征组合
    ↓
Dropout (0.2)             ← 防止过拟合
    ↓
全连接层 (16) + ReLU      ← 压缩到 16 维
    ↓
输出层 (1) + Sigmoid      ← 输出 P(正数)，范围 [0, 1]
```

- **可训练参数**：1089
- **损失函数**：BCELoss（二元交叉熵）
- **优化器**：Adam（lr=0.001）
- **训练轮数**：30 epochs
- **批次大小**：128

## 核心知识点

### 1. 为什么网络能 100% 准确？

16 位有符号整数使用**补码（two's complement）**表示：

- 最高位（MSB，bit15）= 0 → 非负数（0 ~ 32767）
- 最高位（MSB，bit15）= 1 → 负数（-32768 ~ -1）

网络通过训练自动发现：**第一层中连接 MSB 的权重远大于其他位**（约 5-7 倍），说明它学会了"符号位决定正负"这一规律。

### 2. 数据表示

```python
def int_to_bits(value):
    """将 16 位有符号整数转为 16 位二进制特征"""
    unsigned = value & 0xFFFF      # 取低 16 位，保留补码位模式
    bits = [(unsigned >> (15 - i)) & 1 for i in range(16)]
    return np.array(bits, dtype=np.float32)
```

| 十进制 | 16 位补码表示（MSB 在前） | 标签 |
|--------|---------------------------|------|
| 5 | `00000000 00000101` | 正数 (1) |
| -5 | `11111111 11111011` | 负数 (0) |
| 32767 | `01111111 11111111` | 正数 (1) |
| -32768 | `10000000 00000000` | 负数 (0) |

### 3. 训练流程

```
for epoch in range(epochs):
    for batch in data_loader:
        outputs = model(batch_X)       # 前向传播
        loss = criterion(outputs, y)   # 计算损失
        optimizer.zero_grad()          # 清零梯度
        loss.backward()                # 反向传播
        optimizer.step()               # 更新参数
```

## 项目结构

```
dnn-learn-1/
├── model.py                    # 模型定义、设备选择、数据编码
├── train.py                    # 数据生成、训练循环、权重分析
├── inference.py                # 模型加载、预测、交互式推理
├── requirements.txt            # Python 依赖
├── CLAUDE.md                   # Claude Code 项目指引
└── README.md                   # 本文件
```

三个文件各司其职，体现 **模型 / 训练 / 推理** 分离的设计思想：
- `model.py` 是共享核心，不依赖项目内其他文件
- `train.py` 从 `model` 导入，训练后将权重保存到 `sign_classifier.pth`
- `inference.py` 从 `model` 导入，加载 `.pth` 文件进行预测

## 扩展方向

理解本示例后，可以尝试：

1. **简化网络**：去掉隐藏层，用单层感知机（Logistic Regression）解决，观察效果
2. **增加噪声**：在输入位上随机翻转，测试网络鲁棒性
3. **更复杂的任务**：判断数字的奇偶性、是否为 2 的幂、绝对值是否大于某个阈值等
4. **CNN 视角**：将 16 位视为 1D 序列，用 Conv1d 处理
5. **直接输入数值**：改为输入单个归一化后的整数值，对比两种输入方式的区别

## 许可

MIT License
